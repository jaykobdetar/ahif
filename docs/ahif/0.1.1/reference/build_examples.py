...
def row(ns,id,actor,text,kind='post',**kw):
    r={'format_version':'0.1.1','record_key':key(ns,'posts',id),'actor':actor,
     ...
     'provenance':{'locator':{'kind':'json_pointer','value':'/records/0'},'normalizer':{'name':'fictional_fixture_builder','version':'0.1.1'},'transformations':[]}}
...
r['content']={'availability':'empty','completeness':'complete','reason':'Image-only contribution; source explicitly reports no caption.','parts':[]}
...
review=[]
r=row(FORUM,'title_only',c,'')
r['content'].update(completeness='partial',reason='Only title captured; body availability unknown.',parts=[part('A fictional repair question',role='title')])
review.append(r)
r=row(FORUM,'removed_archive',c,'Authorized fictional archived text.')
r['lifecycle']['state']='removed';review.append(r)
r=row(FORUM,'redacted_partial',c,'Retained fictional excerpt.')
r['content'].update(availability='redacted',completeness='partial',reason='Remainder withheld by fixture supplier.')
review.append(r)
r=row(FORUM,'native_link',c,'A destination is supplied outside this text.')
r['links']=[{'link_id':'destination','role':'native_destination','url_as_supplied':'https://destination.example/item',
 'resolved_url':None,'resolution_basis':'not_attempted'}];review.append(r)
for ident in ('same_text_1','same_text_2'):
 review.append(row(FORUM,ident,c,'Two distinct events can contain exactly these words.'))
r=row(FORUM,'repeated',c,'Unchanged text across observations.')
review.append(r)
r=deepcopy(r);r['observed_at']=known('2026-09-02T12:00:00Z');review.append(r)
r=deepcopy(r);r['content']['parts'][0]['text']='Conflicting supplied wording; edit not established.'
r['observed_at']=unknown();review.append(r)
r=row(FORUM,'handle_actor',{**c,'id_type':'handle','id':'FictionalHandle'},'Handle observed in this capture only.')
review.append(r)
r=row(FORUM,'local_record',None,'Source-local anonymous event.')
r['content']['parts'][0]['attribution']['relation']='unknown';review.append(r)
r=row(FORUM,'repost_captured',c,'',kind='repost')
r['content']['parts']=[part('Captured original text belongs to someone else.',role='quote',attr='other')]
review.append(r)
r=row(FORUM,'opaque_extensions',c,'Literal separators: \u2028 \u0085 \u2029 and decomposed e\u0301.')
r['extensions']={'org.example:metadata':{'status':'interval','start':'provider-specific','value':42}}
review.append(r)
...
def make_bundle(name,rows,accounts=False,raw=True):
    ...
    source={'mode':'synthetic','description':'Fictional '+name+' fixture; no live account or network collection.',
     'acquired_at':known(),'content_as_of':unknown('varies_by_record'),
    ...
    for i,r in enumerate(rows):
        ...
        if r['record_key']['id']=='local_record':
            r['record_key'].update(namespace='urn:ahif:source-local',collection='records',id_type='source_local',id=source['source_id']+'#/records/'+str(i))
    ...
    if accounts:
        ...
        for k in keys.values():
            ar={'format_version':'0.1.1','account_key':k,'source_id':source['source_id'],
             ...
             'provenance':{'locator':{'kind':'manual','value':'Fictional alias for schema testing'},
              'normalizer':{'name':'fictional_fixture_builder','version':'0.1.1'},'transformations':[]}}
    ...
    manifest={'format':'AHIF','format_version':'0.1.1','dataset_id':'fictional-'+name,'sources':[source],
     'coverage':[coverage],'files':sorted(files,key=lambda f:f['path'])}
    if name=='review-cases':
        # Same native event from a distinct capture. This descriptor carries no
        # artifact; provenance is a fixture declaration, never a raw-field claim.
        second=deepcopy(source)
        second.update(description='Independent fictional capture descriptor.',artifacts=[],acquired_at=unknown())
        second['source_id']=bound_id('source',second)
        clone=deepcopy(next(r for r in rows if r['record_key']['id']=='repeated'))
        clone['source_id']=second['source_id']
        clone['provenance']['locator']={'kind':'manual','value':'Fictional repeated capture'}
        clone['observation_id']=bound_id('record',clone)
        rows.append(clone);rows.sort(key=lambda r:r['observation_id'])
        data=b''.join(canonical(r)+b'\n' for r in rows)
        (out/'records.jsonl').write_bytes(data)
        next(f for f in files if f['role']=='records').update(sha256=hashlib.sha256(data).hexdigest(),byte_length=len(data),row_count=len(rows))
        second_coverage=deepcopy(coverage);second_coverage['source_id']=second['source_id']
        second_coverage.update(status='unknown',basis='No source completeness evidence.',selection_description='Selected fictional repeat.')
        second_coverage['reported_totals']=[{'name':'reported_posts','value':'1000','unit':'posts','precision':'rounded',
          'as_of':unknown(),'source_field':'fictional_profile_count','scope':'Source inclusion rules unknown.'}]
        manifest['sources']=sorted([source,second],key=lambda s:s['source_id'])
        manifest['coverage']=sorted([coverage,second_coverage],key=canonical)
...
if __name__=='__main__':
    for name,rows,accounts,raw in [('minimal',minimal,False,False),('mixed-platform',mixed,True,True),
          ...
          ('review-cases',review,True,False),('empty-bundle',[],False,False)]:
